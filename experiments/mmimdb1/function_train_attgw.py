...
BATCH_SIZE      = 512
MAX_STEPS       = 1000            # (kept as in your snippet)
VAL_CHECK       = 0.99
...
# Logging / checkpoints
WANDB_ENABLED   = bool(int(os.getenv("WANDB_ENABLED", "0")))   # default off
WANDB_PROJECT   = os.getenv("WANDB_PROJECT", "mmm-imdb-1.0")
WANDB_ENTITY    = os.getenv("WANDB_ENTITY",  "vanrullen-lab")
...
def instantiate_and_train(
    ...
    # Loggers (CSV always; W&B optional via env)
    csv_logger = CSVLogger(save_dir=str(LOG_DIR), name=run_name, flush_logs_every_n_steps=50)
    loggers = [csv_logger]
    if WANDB_ENABLED:
        wb = WandbLogger(
            project=WANDB_PROJECT, entity=WANDB_ENTITY,
            group=f"final-1x-gw-{backbone}-attn-keys",
            tags=["imdb1", "aug70", "gw", backbone, "3mod", "labels", "view=all", SCHED_TAG, f"H{HIDDEN_DIM}", f"keys={keys_tag}"],
            log_model=False, save_dir=str(LOG_DIR), name=run_name, reinit=True, resume=False
        )
        wb.experiment.define_metric("global_step")
        wb.experiment.define_metric("train/*", step_metric="global_step")
        wb.experiment.define_metric("val/*",   step_metric="global_step")
        loggers = [wb, csv_logger]
